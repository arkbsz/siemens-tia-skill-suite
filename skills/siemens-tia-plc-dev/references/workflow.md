...
Use `console` when the workflow needs a native Windows window instead of only terminal commands:

```powershell
powershell.exe -NoProfile -ExecutionPolicy Bypass -File "$env:USERPROFILE\.codex\skills\siemens-tia-plc-dev\scripts\invoke-siemens-plc-dev.ps1" console -ProjectPath "D:\path\to\project"
```

The default `console` route builds and opens `PLCDevConsole.exe`. It provides an IDE-style top menu, automatic current-TIA-project detection, a compact one-line project toolbar, a scrollable main canvas with right-side vertical scrollbar and mouse-wheel support, a draggable left project tree, switchable center pages for AI chat/log/file preview/runs/reference image, common Openness workflow buttons, a `write-cycle` input box for generated LAD XML, and a bottom AI task composer focused on the user request. The AI area includes a horizontally scrollable quick-configuration row for model, workflow, language preference, TIA session mode, safety mode, and step timeout. Advanced API, image, WinCC component, and font settings remain under `Tools > AI / API / Image / WinCC Settings`. All settings persist to `PLC_Code\config\ai-workflow.json`; task drafts reference it, and `read-cycle`/`write-cycle` receive it and archive a snapshot in their run output. For WinCC tasks, the composer can upload a reference image, preview it, auto-route text-to-image or image-to-image work, and save component-mapping prompts for WinCC-native implementation under `PLC_Code\ai-prompts`. Use `console-web` only as a fallback browser UI.
