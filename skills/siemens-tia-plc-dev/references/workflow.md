...
The default `console` route builds and opens `PLCDevConsole.exe`. It provides automatic current-TIA-project detection, browse/open actions, a draggable left project tree, switchable center pages for AI chat/log/file preview/runs/reference image, common Openness workflow buttons, a `write-cycle` input box for generated LAD XML, and a bottom AI task composer with model/workflow/API/image/component/font settings. For WinCC tasks, the composer can upload a reference image, preview it, auto-route text-to-image or image-to-image work, and save component-mapping prompts for WinCC-native implementation under `PLC_Code\ai-prompts`. Use `console-web` only as a fallback browser UI.
