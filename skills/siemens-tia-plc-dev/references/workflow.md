...
The default `console` route builds and opens `PLCDevConsole.exe`. It provides an IDE-style top menu, automatic current-TIA-project detection, a compact one-line project toolbar, a scrollable main canvas with right-side vertical scrollbar and mouse-wheel support, a draggable left project tree, switchable center pages for AI chat/log/file preview/runs/reference image, common Openness workflow buttons, a `write-cycle` input box for generated LAD XML, and a bottom AI task composer focused on the user request. Model, API, image, WinCC component, and font settings live under `Tools > AI / API / Image / WinCC Settings` in a scrollable drop-down panel so the bottom interaction area does not collide with configuration controls. For WinCC tasks, the composer can upload a reference image, preview it, auto-route text-to-image or image-to-image work, and save component-mapping prompts for WinCC-native implementation under `PLC_Code\ai-prompts`. Use `console-web` only as a fallback browser UI.
