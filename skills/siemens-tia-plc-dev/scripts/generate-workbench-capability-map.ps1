...
Add-Capability $items "wincc-visual" "WinCC 参考图/文字到可编辑画面工程包" (Test-PathState $winccPackage "PARTIAL_READY" "PARTIAL") "上传参考图 / WinCC方案 / WinCC插件 / 参考图 Tab" "wincc-plugins, wincc-visual-package, imagegen route, agent-plan" "design brief, screen map, component map, tag contract, style guide" "plugin routing, component-selection matrix, HMI safety review" "视觉生成仍是设计输入，最终对象写入必须走真实 WinCC Openness 路由" "继续扩展对象级画面组件和参考图到原生控件的映射。"
...
Add-Capability $items "agent-context-injection" "Agent 工程上下文注入与证据清单" (Test-PathState $contextManifestScript "READY" "PARTIAL") "AI对话 / 队列阶段 / 任务状态" "prepare-agent-context, agent-chat -ContextManifest, queue-run-current" "agent-sessions\\*\\context-manifest.json, console-jobs 上下文字段, platform.selected 事件" "路径边界校验、上下文文件计数、重试/继续输入证据复制、离线路由回归测试" "上下文仍是导出文件和工作台证据，不替代 TIA 编译器或生产发布审批" "按任务类型进一步缩小上下文范围，并增加哈希绑定和上下文版本差异。"
...
Add-Capability $items "review-release" "PR 式审查、验证总览与发布边界" $reviewStatus "审查包 / 发布审批 / 总览面板 / 验证面板 / Diff" "review-package, review-approval, workbench-dashboard, prepare-release, apply-release" "review-summary, artifact-index, import-readiness, approval.json, release-manifest.json, dashboard" "hash, diff, readiness, clone compile result, approval status and expiry" "生产导入和下载必须明确由用户触发" "增加多用户签名、审批角色和审计日志。"
...
Add-Capability $items "simulation-replay" "仿真场景回放_SimulationReplay" (Test-PathState $simulationReplay "PARTIAL_READY" "PARTIAL") "仿真回放 / 验证面板 / 自动流水线" "simulation-replay" "replay-report.md/json, scenario-results.csv" "offline scenario evidence matrix, latest write-cycle evidence lookup, GraphQL/PLCSIM readiness checks" "真实 PLCSIM Advanced API 驱动、WinCC Runtime 联机冒烟和设备 I/O 激励仍需本机运行环境" "把 replay 状态机升级为可调用 PLCSIM Advanced / Runtime adapter 的执行器。"
...
$json = [pscustomobject]@{
    ...
    readiness = [pscustomobject]@{
        ...
        winccOpennessImplementation = (Test-Path -LiteralPath $winccOpennessImplementation)
        simulationPackage = (Test-Path -LiteralPath $simulationPackage)
        simulationReplay = (Test-Path -LiteralPath $simulationReplay)
