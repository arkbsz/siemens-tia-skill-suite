...
5. Use TIA Add-In ShowScripts on compatible Unified projects to export screen JavaScript for review; it is an audit/export adapter, not a general screen writer.
6. Use the WinCC Unified GraphQL MCP only for runtime validation after its endpoint and local server path are configured.
7. Use Custom Web Controls only when standard controls and reusable faceplates cannot meet the requirement. Verify license and source before copying community controls.
